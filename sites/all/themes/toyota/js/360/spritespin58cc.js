    ...
    data.preload.fadeIn(250, function(){
      SpriteLoader.preload(data.source, function(images){
        data.preload.fadeOut(250, function() {
            data.preload.hide();
                        //TIME Modify 2016.10.13
                        $("#loading").fadeOut(250);
        });
//        data.stage.show();
        data.stage.fadeIn(250);
        if (data.canvas){
//          data.canvas.show();
          data.canvas.fadeIn(250);
        }
        data.images = images;
  ...
  Spin.prepareBackground = function(data){
    var w = [data.width * (data.dispWidth / data.width), "px"].join("");
    var h = [data.height * (data.dispHeight / data.height), "px"].join("");
  ...
  $.fn.spritespin = function(method) {
    ...
    return this.each(function(){
      ...
      if (!data){
        ...
        if (options.source || (options.dispWidth && options.dispHeight)){
          // tokita adding
  ...
  api.update = function(frame){
    return this.each(function(){
            //切替時のローディング画像の設定 2016.10.20 Modify
      frameNo = frame;
      var $this = $(this);
      var data = $this.data('spritespin');
...
(function($, window, Spin){
  Spin.behaviors.drag = {
    ...
    mouseup    : function(e){ 
      ...
      // tokita add
      // mouse up update
      $this.spritespin("update", data.frame);  // update to frame
    },
...
(function($, window) {
  ...
  Module.reload = function(data){
    ...
    data.modopts = {
      ...
      dispWidth : data.dispWidth / data.width,
      dispHeight: data.dispHeight / data.height,
  ...
  Module.drawSpritesheet = function(data){
    ...
    // tokita fixed
    data.stage.css({
      width      : [data.width * data.modopts.dispWidth, "px"].join(""),
      height     : [data.height * data.modopts.dispHeight, "px"].join(""),
      "background-image"    : ["url('", image, "')"].join(""),
      "background-repeat"   : "no-repeat",
      "background-position" : [-x * data.modopts.dispWidth, "px ", -y * data.modopts.dispHeight, "px"].join(""),
      ...
      "-webkit-background-size" : [opts.resX * data.modopts.dispWidth, "px ", opts.resY * data.modopts.dispHeight, "px"].join(""),
      "background-size" : [opts.resX * data.modopts.dispWidth, "px ", opts.resY * data.modopts.dispHeight, "px"].join("")
    });
  };
  ...
  Module.drawImages = function(data){
    ...
    var frame = data.stage.css({
      width      : [data.dispWidth, "px"].join(""),
      height     : [data.dispHeight, "px"].join(""),
    }).children().hide()[data.frame];
    // tokita add
    $(frame).css('width', data.dispWidth);
    $(frame).css('height', data.dispHeight);
