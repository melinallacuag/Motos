...
function junotoys_ready_actions() {
	...
	// Prepare menus
	if (JUNOTOYS_STORAGE['menu_cache']) junotoys_prepare_menus();
	...
	jQuery('.popup_form.login_form').submit(function(e){
		"use strict";
		var rez = junotoys_login_validate(jQuery(this));
		if (!rez)
			e.preventDefault();
		return rez;
	});
	...
	jQuery('.popup_form.registration_form').submit(function(e){
		"use strict";
		var rez = junotoys_registration_validate(jQuery(this));
		if (!rez)
			e.preventDefault();
		return rez;
	});
	...
	jQuery("form#commentform").submit(function(e) {
		"use strict";
		var rez = junotoys_comments_validate(jQuery(this));
		if (!rez)
			e.preventDefault();
		return rez;
	});
...
function junotoys_scroll_actions() {
	...
	if (JUNOTOYS_STORAGE['top_panel_height_'] < 1) {
		JUNOTOYS_STORAGE['top_panel_height'] = Math.max(0, jQuery('.top_panel_').height());
	}
	...
	// Fix/unfix top panel
	if (!jQuery('body').hasClass('menu_mobile_') && JUNOTOYS_STORAGE['menu_fixed']) {
		var slider_height = 0;
...
function junotoys_prepare_menus() {
	"use strict";
	var menus = [
		jQuery('ul#menu_main'),
		jQuery('ul#menu_user'),
